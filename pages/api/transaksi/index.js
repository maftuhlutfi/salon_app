...
async function handler(req, res) {
    ...
    if (req.method == 'POST') {
        ...
        try {
            await query(`INSERT INTO transaksi (id_pelanggan, tipe_bayar, alamat_kirim) VALUES (?,?,?)`, [id_pelanggan, tipe_bayar, alamat_kirim])
            return res.send('Transaksi berhasil ditambahkan.')
        } catch (e) {
            console.log(e)
            res.status(500).send('Terjadi kesalahan. Coba lagi beberapa saat.')
        }
